...
**Critical**: "In" and "Out" are from the **firewall's perspective on the interface where the rule matches**. For a rule on VLAN 10:
- **In** = traffic from VLAN 10 hosts → firewall → WAN (upload)
- **Out** = traffic from WAN → firewall → VLAN 10 hosts (download)
