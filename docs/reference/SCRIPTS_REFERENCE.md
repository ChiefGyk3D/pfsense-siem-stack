...
|--------|---------|-------|-------------|
| **preflight.sh** | Pre-install/deploy sanity checks | `./preflight.sh` | Before install/setup |
...
**Purpose:** Validate configuration and connectivity BEFORE running `install.sh` or `setup.sh`

**What it checks:**
- ✓ `config.env` exists and defines `SIEM_HOST`, `PFSENSE_HOST`, `PFSENSE_USER`, `SIEM_SSH_USER`
- ✓ SSH key-based access to pfSense (hard failure) and the SIEM server (warning)
- ✓ `python3` available on pfSense (required by the forwarder)
...
**Usage:**
```bash
./scripts/preflight.sh          # standalone
./setup.sh                      # runs preflight automatically
./setup.sh --skip-preflight     # escape hatch
```

Makes no changes to any host; exits non-zero when a hard check fails. Also available as
option 21 in the [`pfsense-siem` management console](../operations/MANAGEMENT_CONSOLE.md).
