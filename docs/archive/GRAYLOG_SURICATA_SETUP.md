...
scp scripts/install_graylog_opensearch.sh user@192.0.2.10:~/
...
1. Open a browser and navigate to: `http://192.0.2.10:9000`
2. Login with:
   - **Username**: `admin`
...
2. Under **Remote Logging Options**:
   ...
   - **IP Protocol**: IPv4
   - **Remote log servers**: `192.0.2.10:514`
...
tail -F /var/log/suricata/*/eve.json | \
    while read line; do
        echo "$line" | nc -u 192.0.2.10 12201
    done &
EOF
...
- Test from pfSense: `echo "test message" | nc -u 192.0.2.10 514`
- Check pfSense firewall rules allow outbound to 192.0.2.10
...
- Test connectivity: `curl http://192.0.2.10:9200`
- Check OpenSearch security settings (should be disabled for local access)
- Verify index pattern exists: `curl http://192.0.2.10:9200/_cat/indices?v`
