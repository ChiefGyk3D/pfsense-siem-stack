...
1. Open Grafana: http://192.0.2.10:3000
...
curl -X POST \
  ...
  http://192.0.2.10:3000/api/dashboards/db
```
...
**Checklist**:
1. Verify OpenSearch is running: `curl http://192.0.2.10:9200/_cat/indices/suricata-*`
...
5. Confirm field mappings: `curl http://192.0.2.10:9200/suricata-*/_mapping | jq`
...
### Check Event Count
```bash
curl -s http://192.0.2.10:9200/suricata-*/_count | jq
```

### Verify Field Mappings
```bash
curl -s http://192.0.2.10:9200/suricata-*/_mapping | \
  jq '.[] | .mappings.properties.suricata.properties.eve.properties | keys'
```

### Test Protocol Aggregation (Pie Chart Query)
```bash
curl -s -X POST http://192.0.2.10:9200/suricata-*/_search -H 'Content-Type: application/json' -d '{
  "size": 0,
  "aggs": {
    "protocols": {
      "terms": {
...
curl -H "Authorization: Bearer YOUR_API_KEY" \
  http://192.0.2.10:3000/api/dashboards/uid/suricata-improved-v2 | \
