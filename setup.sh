...
if ! curl -s -f -u "$GRAFANA_AUTH" "${GRAFANA_URL}/api/health" > /dev/null 2>&1; then
    ...
    if ! curl -s -u "$GRAFANA_AUTH" "${GRAFANA_URL}/api/plugins/grafana-opensearch-datasource" | grep -q '"id"' 2>/dev/null; then
        ...
        if command -v grafana-cli > /dev/null 2>&1; then
            ...
            sleep 5
        elif ssh -o BatchMode=yes "user@${SIEM_HOST}" 'command -v grafana-cli' > /dev/null 2>&1; then
            ssh "user@${SIEM_HOST}" 'sudo grafana-cli plugins install grafana-opensearch-datasource 2>/dev/null && sudo systemctl restart grafana-server' 2>/dev/null || true
            sleep 5
        else
