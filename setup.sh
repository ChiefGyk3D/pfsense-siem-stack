...
PFSENSE_PYTHON=$(ssh "${PFSENSE_USER}@${PFSENSE_HOST}" \
    'for p in /usr/local/bin/python3 /usr/local/bin/python3.13 /usr/local/bin/python3.12 /usr/local/bin/python3.11 /usr/bin/python3; do [ -x "$p" ] && echo "$p" && break; done')
...
# Bake an explicit GeoIP path if the user set one
if [[ -n "${GEOIP_DB_PATH:-}" ]]; then
    sed -i "s|^if os.getenv(\"GEOIP_DB_PATH\"):|os.environ.setdefault(\"GEOIP_DB_PATH\", \"${GEOIP_DB_PATH}\")\nif os.getenv(\"GEOIP_DB_PATH\"):|" "$TEMP_FORWARDER"
fi
...
# Deploy watchdog — restarts the rc.d service if the forwarder process is gone.
# Runs from root's crontab every minute (see below).
...
#!/bin/sh
# Auto-generated by pfsense-siem-stack setup.sh — restarts forwarder if not running
...
# (rc.start_packages). The file MUST end in .sh or it is never run at boot.
# pfSense does not manage /etc/rc.conf, so the script enables itself by default.
info "Installing rc.d service for boot persistence..."
RCD_SCRIPT="/usr/local/etc/rc.d/suricata_forwarder.sh"
ssh "${PFSENSE_USER}@${PFSENSE_HOST}" "cat > ${RCD_SCRIPT}" << 'RCD_EOF'
#!/bin/sh
# PROVIDE: suricata_forwarder
# REQUIRE: DAEMON
# KEYWORD: shutdown
#
# Managed by pfsense-siem-stack setup.sh — re-run setup.sh to regenerate
# (for example after a pfSense upgrade changes the Python interpreter).

. /etc/rc.subr

name="suricata_forwarder"
rcvar="suricata_forwarder_enable"
python="__PFSENSE_PYTHON__"
command="/usr/local/bin/forward-suricata-eve.py"
command_interpreter="${python}"
pidfile="/var/run/${name}.pid"              # daemon(8) supervisor
child_pidfile="/var/run/${name}.child.pid"  # the forwarder itself
...
suricata_forwarder_start() {
    ...
    /usr/sbin/daemon -f -P "$pidfile" -p "$child_pidfile" -o "$logfile" -r "$python" "$command"
    echo "${name} started."
}
...
load_rc_config $name
: ${suricata_forwarder_enable:="YES"}
run_rc_command "$1"
RCD_EOF

# Bake in the detected interpreter, remove the pre-.sh unit if present (it was
# never started at boot), and make the script executable.
ssh "${PFSENSE_USER}@${PFSENSE_HOST}" "sed -i '' 's|__PFSENSE_PYTHON__|${PFSENSE_PYTHON}|' ${RCD_SCRIPT} && chmod 755 ${RCD_SCRIPT} && rm -f /usr/local/etc/rc.d/suricata_forwarder"
info "rc.d service installed (${RCD_SCRIPT}, interpreter ${PFSENSE_PYTHON})"
...
if [[ -n "$FORWARDER_PID" ]]; then
    ...
    echo "  Check logs: ssh ${PFSENSE_USER}@${PFSENSE_HOST} 'tail -50 /var/log/suricata-forwarder.log; tail -50 /var/log/system.log | grep suricata'"
    ERRORS=$((ERRORS+1))
fi
...
echo "    ssh ${PFSENSE_USER}@${PFSENSE_HOST} 'service suricata_forwarder.sh status'"
echo ""
