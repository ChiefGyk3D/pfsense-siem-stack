...
DEBUG_LOG = "/var/log/suricata_forwarder_debug.log"
                ...
                if line:
                    ...
                    if line:
                        try:
                            ...
                            # Normalize interface name (remove Netmap direction indicators)
                            # Netmap appends ^ for egress, * for host stack, etc.
                            # Strip these to get clean interface names in Grafana/Opensearch
                            if "in_iface" in event:
                                event["in_iface"] = event["in_iface"].rstrip('^*+-')
